...
/*** DOCUMENTATION
	<application name="Dev189Relay" language="en_US">
		<synopsis>
			Record a call and mix the audio during the recording. Use of StopDev189Relay is required
		...
		<see-also>
			<ref type="application">Dev189Relay</ref>
			<ref type="application">StopDev189Relay</ref>
			<ref type="function">AUDIOHOOK_INHERIT</ref>
		</see-also>
	</application>
	<application name="StopDev189Relay" language="en_US">
		<synopsis>
			Stop recording a call through Dev189Relay, and free the recording's file handle.
		</synopsis>
		<syntax>
			<parameter name="Dev189RelayID" required="false">
				<para>If a valid ID is provided, then this command will stop only that specific
				MixMonitor.</para>
			</parameter>
		</syntax>
		<description>
			<para>Stops the audio recording that was started with a call to <literal>Dev189Relay()</literal>
...
static const char *const app = "Dev189Relay";

static const char *const stop_app = "StopDev189Relay";

static const char *const mixmonitor_spy_type = "Dev189Relay";
...
static void mixmonitor_ds_close_fs(struct mixmonitor_ds *mixmonitor_ds)
{
	...
	if (mixmonitor_ds->fs)
	{
		...
		ast_verb(2, "Dev189Relay close filestream (mixed)\n");
	}
	...
	if (mixmonitor_ds->fs_read)
	{
		...
		ast_verb(2, "Dev189Relay close filestream (read)\n");
	}
	...
	if (mixmonitor_ds->fs_write)
	{
		...
		ast_verb(2, "Dev189Relay close filestream (write)\n");
	}
...
static void copy_to_voicemail(struct mixmonitor *mixmonitor, const char *ext, const char *filename)
{
	...
	AST_LIST_TRAVERSE(&mixmonitor->recipient_list, recipient, list)
	{
		...
		ast_verb(4, "Dev189Relay attempting to send voicemail copy to %s@%s\n", recording_data.mailbox,
...
static void *mixmonitor_thread(void *obj)
{
	...
	ast_verb(2, "Begin Dev189Relay Recording %s\n", mixmonitor->name);
	...
	ast_verb(2, "End Dev189Relay Recording %s\n", mixmonitor->name);
	ast_test_suite_event_notify("DEV189RELAY_END", "File: %s\r\n", mixmonitor->filename);
	...
	if (!AST_LIST_EMPTY(&mixmonitor->recipient_list))
	{
		...
		{
			ast_verb(3, "Copying recordings for Dev189Relay %s to voicemail recipients\n", mixmonitor->name);
			copy_to_voicemail(mixmonitor, fs_ext, mixmonitor->filename);
		}
		if (!ast_strlen_zero(fs_read_ext))
		{
			ast_verb(3, "Copying read recording for Dev189Relay %s to voicemail recipients\n", mixmonitor->name);
			copy_to_voicemail(mixmonitor, fs_read_ext, mixmonitor->filename_read);
		}
		if (!ast_strlen_zero(fs_write_ext))
		{
			ast_verb(3, "Copying write recording for Dev189Relay %s to voicemail recipients\n", mixmonitor->name);
...
static int mixmonitor_exec(struct ast_channel *chan, const char *data)
{
	...
	{
		ast_log(LOG_WARNING, "Dev189Relay requires an argument (filename or ,t(filename) and/or r(filename)\n");
		return -1;
	}
	...
	{
		ast_log(LOG_WARNING, "Dev189Relay requires an argument (filename)\n");
		return -1;
	}
...
AST_MODULE_INFO_STANDARD(ASTERISK_GPL_KEY, "Dev189 Relay Application");
